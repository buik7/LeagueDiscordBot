...
const defaultChampion: ChampionProps = {
  name: "Unknown champion",
  discordIcon: "<:NONE:1062425584187224187>",
};

/**
 * Get champion props by id
 * @param id Champion id
 * @param capitalName whether to convert name to user-friendly format
 * @returns ChampionProps
 */
export const getChampionById = (
  id: number,
  capitalName: boolean = false
): ChampionProps => {
  ...
  if (capitalName) {
    champion.name = convertChampionNameToCapital(champion.name);
  }
  return champion;
};
 ...
 */
export const convertChampionNameToCapital = (championName: string) => {
